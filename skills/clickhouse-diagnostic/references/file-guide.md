...
### system.query_views_log_3_days
**Why we run it:** the only place a materialized view's own execution is recorded. An MV that throws, or that "succeeds" while writing nothing, is invisible in `query_log` — the parent INSERT reports success either way.
**Question:** did every MV fire for every insert, did any fail after the base part was already committed, and what does each cost in memory and time?
**Read first:** `status` totals (`QueryFinish` vs `ExceptionWhileProcessing`); `zero_write_executions / executions` per view; `written_rows` per hour compared across views on the same source; `exception_code` with its `exception`; `median_peak_memory_usage` and `max_view_duration_ms` per view over time.
**Healthy looks like:** every row `QueryFinish` with `exception_code = 0`; `zero_write_executions` either near 0 or a stable fraction explained by the MV's WHERE clause; per-view memory flat across days.
**Red flags:** any `ExceptionWhileProcessing` — the base insert committed and the MV target did not (P-36); `zero_write_executions ≈ executions` with `read_rows > 0` on a view that used to write (P-35); one view's `written_rows` per hour diverging from a sibling on the same source (P-35); `median_peak_memory_usage` stepping up several-fold across an upgrade boundary while `written_rows` stays flat (P-54); `max_view_duration_ms` dominating the insert path (P-34).
**Traps:** **a failed INSERT marks EVERY view in the pipeline with `ExceptionWhileProcessing` and the same message, not just the one that broke** — attribute the failure to the view named in the text ("while pushing to view X"), which is normally also the one with the highest count; sums are per **1-hour** bucket, so divide by `executions` for averages; `zero_write_executions > 0` is normal for a filtering MV, so always read it as a ratio against that view's own earlier buckets; `view_uuid` is not collected because the server writes the zero UUID into it — for an MV declared without `TO`, `view_target` is `db.`.inner_id.<uuid>`` and that *does* change when the view is recreated; an absent or empty file means `<query_views_log>` is not configured or `log_query_views = 0`, which is a collection gap, not a healthy signal; gov has no `exception` text and splits the identifiers into hashed database/table pairs.
**Pairs with:** `system.tables` (MV DDL and dependency chain), `query_log_details` `Insert` rows for the source, `part_log` NewPart on the target, `asynchronous_insert_log` (flush latency driven by MV fan-out).
