...
| 1.4 | `system.replicas` | `is_readonly = 1` | **critical** (alert `replica_readonly`) | Replica cannot accept inserts: Keeper session lost (`is_session_expired = 1`), metadata mismatch, or disk full. → HC-3, HC-4, P-40. |
...
| 3.10 | `system.zookeeper_log_1_day` | Response rows with `error IN (ZSESSIONEXPIRED, ZCONNECTIONLOSS, ZOPERATIONTIMEOUT)`; `sessions > 2` in an hour; `p99_duration_ms` ×10 vs baseline | warning→critical | Keeper-side view of the same outage: which operations failed, how many sessions this server burned, whether latency degraded before the failure. Absent file = table not enabled. |
...
| `zookeeper_log.p99_duration_ms` ×10 in the hours *before* the failures | **Keeper saturated first** — latency rose, then sessions timed out | Keeper memory/disk/snapshot pressure, request volume (`requests/h` by `op_num`), `Replicated` database count × replicas (HC-3.13) |
