...
| `system.tables` | `database, name, uuid, engine, is_temporary, data_paths, metadata_path, metadata_modification_time, dependencies_*, create_table_query, engine_full, as_select, partition_key, sorting_key, primary_key, sampling_key, storage_policy, comment, has_own_data, loading_dependencies_*, loading_dependent_*` (+ `total_rows, total_bytes`; `metadata_version` ≥ 24.2, `parameterized_view_parameters` ≥ 25.4, `target_database, target_table` ≥ 26.6) | hashed `database`, `name`, plus `uuid, engine, is_temporary, metadata_modification_time, storage_policy, has_own_data, total_rows, total_bytes` and the four `dependencies_*` / `loading_dependencies_*` arrays hashed element by element (+ `metadata_version` ≥ 24.2, hashed `target_*` ≥ 26.6). No DDL, no keys. | Full DDL and ORDER BY live here — the source for schema-design findings. **Credentials in `create_table_query` / `engine_full` / `as_select` read `'[HIDDEN]'`**: masked by the server on ≥ 23.x and by the collector's redactor for anything older or missed (AWS key ids, `SETTINGS … password = …`). |
| `system.columns` | `database, table, name, type, position, default_kind, default_expression, is_in_primary_key, is_in_sorting_key` | hashes database/table/name; no `default_expression`; the two flags stay | Large (one row per column). The key flags say whether a filter column is in the key without parsing `sorting_key`. |
...
| `system.view_refreshes` (≥ 23.12) | `database, view, status, last_success_time, next_refresh_time, exception` (500 chars; cloud adds `hostName()` and reads every replica) | hashed `database`/`view`; `has_exception` instead of the text | One row per refreshable materialized view (`REFRESH EVERY … / AFTER …`). `status` ∈ Scheduled, Running, Disabled, WaitingForDependencies, RunningOnAnotherReplica. These views never appear in `query_views_log`; a stale `last_success_time` or a non-empty `exception` is the only sign one has stopped. Absent below 23.12. |
...
Self-contained; all panel data is embedded as one JSON literal on a line starting `const DATA = {`. Keys: `alerts` (full rule results **including matched rows**), `keeper_metric_hourly` (per hour: `transactions, hw_exceptions, user_exceptions, wait_us, sessions_min, sessions_max` — the Keeper health test inputs, 7 days; **check `keeper_wait_available` / `keeper_session_available` first** — when either is `false` the server's version does not export that counter and the generator substituted `0`, which is indistinguishable from a real zero, so never read `sessions_min = 0` as a lost session or `wait_us = 0` as zero latency without them), `keeper_errors_hourly` (`time, code_name, count` for 999/242/319/571/252; `keeper_errors_source` says whether it came from `error_log` or `query_log`), `keeper_connection` (live `zookeeper_connection` rows), `version, uptime, total_databases, total_tables, active_parts, total_size`, `host_info`, `host_checks`, `storage_by_db`, `engines_dist`, `tables_list`, `query_by_time`, `query_by_kind`, `query_slow`, `query_heavy`, `query_by_user`, `exceptions`, `part_log_by_time`, `part_log_by_type`, `dictionaries`, `crash_log`, `mutations`, `detached`, `replication_queue`, `clusters`, `replicas`, `disks`, `text_log`, `bundle_files`, `server_errors`, `high_part_count`, `ttl_activity`, `async_inserts`, `schema_graph` (`file, tables, columns, databases, mvs, refreshable, dictionaries` — present only when `schema_graph.html` was written), and `qa_*` when query analysis ran. Extraction recipe in `reading-recipes.md`.

## 8b. `schema_graph.html`

The table-dependency graph, adapted from ClickHouse's `/schema` page and rendered entirely from a JSON literal on the line starting `const DATA = {`. Keys: `tables` (`database, name, engine, engine_full, create_table_query, sorting_key, primary_key, partition_key, sampling_key, total_rows, total_bytes, comment, target_database, target_table, dependencies_*, loading_dependencies_*` — system databases excluded), `columns` (`database, table, name, type, is_key, has_default`), `dictionaries` (`database, name, source`), `refreshes` (`database, view, status, last_success_time, next_refresh_time, exception`), `has_target` (false below 26.6: MV → target edges are then inferred from `dependencies_*` and miss the implicit `.inner_id.*` table), `version`, `generated_at`. The DDL here passed through the same `[HIDDEN]` redactor as the JSONL. `dashboard.html` loads this file into a frame on the first click of its Schema tab; opened alone it works the same and shows a link back. Both pages follow one theme through `localStorage`. **Never fetched by the pre-pass** — read `system.tables_*.jsonl` for the same facts as data.
