...
| `system.storage_policies` | `policy_name, volume_name, volume_priority, disks, max_data_part_size, move_factor, prefer_not_to_merge` | names hashed (same salt as `system.disks.name`) | Join `disks` with `system.disks.type` (`ObjectStorage` vs `Local`) to know whether table data lives on S3/Azure/GCS. |
| `system.zookeeper_connection` (≥ 23.8; cloud root) | `name, host, port, index, connected_time, session_uptime_elapsed_seconds, is_expired, keeper_api_version, client_id, enabled_feature_flags` (cloud adds `hostname`) | name/host hashed; no port/client_id/flags | One row per Keeper connection. `session_uptime_elapsed_seconds` ≪ server `Uptime` = the session was re-established (expiry / Keeper leader change); `is_expired = 1` = no session now. Cloud: compare `host` and session age across replicas. |
...
| `system.zookeeper_log_1_day` (only when `<zookeeper_log>` is configured) | 1-hour × `type` (Request/Response/Finalize) × `op_num` × `error`; `requests`, `sessions` (`uniq(session_id)`), `p99_duration_ms`, `max_duration_ms` (cloud adds `hostname`; ≤ 20 000) | same | `error` is NULL on Request rows; on Response rows `ZOK` or the Keeper error (`ZSESSIONEXPIRED`, `ZCONNECTIONLOSS`, `ZOPERATIONTIMEOUT`, `ZNONODE`, `ZBADVERSION` …). `sessions` jumping above 1–2 per hour = session churn. Absent file = table not enabled (not evidence of health). |
